				...
				<tr>
					...
					<td>
						...
						if ($this->session->userdata('level')=='10' and $this->uri->segment(4) == 'on_progress' ) {
							$cek = $this->db->get_where('header_visit_outlet', array('group_visit'=>$rw->group_visit,'id_user'=>$this->session->userdata('id_user')));
							if ($cek->num_rows() > 0) {
								$id_visit_outlet = $cek->row()->id_visit_outlet;
								?>
